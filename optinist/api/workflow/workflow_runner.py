...
class WorkflowRunner:
    @classmethod
    def create_workflow(cls, unique_id, name, nodeList, edgeList, nwbParam):
    ...
    @classmethod
    def rulefile(cls, unique_id, nodeList, edgeList, nwbParam):
        ...
        for node in nodeDict.values():
            if node.type == NodeType.IMAGE:
                rule_dict[node.id] = SmkSetfile.imagefile(unique_id, node, edgeList, nwbfile)
            elif node.type == NodeType.CSV:
                rule_dict[node.id] = SmkSetfile.csvfile(unique_id, node, edgeList, nwbfile)
            elif node.type == NodeType.FLUO:
                rule_dict[node.id] = SmkSetfile.csvfile(unique_id, node, edgeList, nwbfile)
            elif node.type == NodeType.BEHAVIOR:
                rule_dict[node.id] = SmkSetfile.csvfile(unique_id, node, edgeList, nwbfile, "behavior")
            elif node.type == NodeType.HDF5:
                rule_dict[node.id] = SmkSetfile.hdf5file(unique_id, node, edgeList, nwbfile)
            elif node.type == NodeType.ALGO:
                rule = SmkSetfile.algofile(unique_id, node, edgeList, nodeDict)

...
from optinist.api.snakemake.snakemake_executor import get_dependencies_graph
...
def test_get_dependencies_graph():
    ...
    test_dir = os.path.join(DIRPATH.ROOT_DIR, 'test_data')
    config_filepath = os.path.join(DIRPATH.ROOT_DIR, 'config.yaml')
    test_config_filepath = os.path.join(test_dir, 'config.yaml')
        ...
        for key_name in ["input", "output"]:
            if isinstance(rule[key_name], list):
                for x in rule[key_name]:
                    input_list.append(os.path.join(test_dir, "snakemake", x))
            else:
                input_list = os.path.join(test_dir, "snakemake", rule[key_name])
    ...
    new_config = FlowConfig(
        last_output=[
            os.path.join(test_dir, "snakemake", x) 

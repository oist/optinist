...
from optinist.api.nwb.nwb import save_nwb, NWBDATASET
from optinist.api.snakemake.snakemake import Rule
from optinist.api.pickle.pickle_writer import PickleWriter
...
def change_dict_key_exist(input_info, rule_config):
...
def read_input_info(input_files):
    input_info = {}
...
def run_function(path, params, input_info):
    wrapper = dict2leaf(
        wrapper_dict,
        path.split('/')
    )
...
def run_script(__rule_config: Rule, last_output):
    try:
        input_info = read_input_info(__rule_config.input)

        change_dict_key_exist(input_info, __rule_config)

        for key in list(input_info):
            if key != "nwbfile" and key not in __rule_config.return_arg.values():
                input_info.pop(key)

        output_info = run_function(__rule_config.path, __rule_config.params, input_info)
